...
class OrderBookGuard:
    ...
    def _fetch_order_book(self, token_id):
        ...
        try:
            ...
            logging.warning("OrderBookGuard: Failed to fetch book for %s: %s", str(token_id)[:16], exc)
            return None
    ...
    def check_before_entry(self, token_id, side="BUY", intended_size_usdc=10.0):
        ...
        if not analysis["book_available"]:
            ...
            logging.warning("OrderBookGuard: BLOCKED %s — no order book found", str(token_id)[:16])
            return check
        ...
        if analysis["bid_depth"] < self.min_bid_depth:
            ...
            logging.warning("OrderBookGuard: BLOCKED %s — only %d bid levels", str(token_id)[:16], analysis["bid_depth"])
            return check

        if analysis["ask_depth"] < self.min_ask_depth:
            check["reason"] = f"insufficient_ask_depth ({analysis['ask_depth']} < {self.min_ask_depth})"
            logging.warning("OrderBookGuard: BLOCKED %s — only %d ask levels", str(token_id)[:16], analysis["ask_depth"])
            return check
        ...
        if spread is not None and spread > self.max_spread:
            ...
            logging.warning(
                ...
                str(token_id)[:16], spread, self.max_spread,
            )
            return check
